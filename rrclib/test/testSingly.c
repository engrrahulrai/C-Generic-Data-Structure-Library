...
{
s=getNextElementFromSinglyLinkedList(iterator);
if(num==s->rollNumber)
{
found=1;
break;
...
void editStudent(singlyLinkedList *list)
{
int roll,found;
singlyLinkedListIterator *iterator;
struct student *s;
...
void insertStudent(singlyLinkedList *list)
{
int nu,success,pos;
struct student *s;
singlyLinkedListIterator *iterator;
...
s->rollNumber=nu;
strcpy(s->name,nam);
insertAtSinglyLinkedList(list,s,pos,&success);
if(!success)
{
free(s->name);
free(s);
...
void removeStudent(singlyLinkedList *list,int pos)
{
int found,j,success;
singlyLinkedListIterator *iterator;
struct student *s;
...
j--;
s=removeAtSinglyLinkedList(list,j,&success);
if(success==false)
{
...
void displayStudent(singlyLinkedList *list)
{
struct student *t;
singlyLinkedListIterator *iterator;
if(list==NULL) return;
...
while(hasMoreElementInSinglyLinkedList(iterator))
{
t=getNextElementFromSinglyLinkedList(iterator);
printf("%d->%s\n",t->rollNumber,t->name);
}
